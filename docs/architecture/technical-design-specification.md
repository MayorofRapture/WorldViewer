...
Initial repository SDK authority:

• JSON-safe public data uses `JsonPrimitive`, `JsonValue`, and `JsonObject` as defined by the Interface & Contract Specification; public values contain no `undefined`, functions, symbols, or class instances, and validated numeric values are finite. Arrays and objects are read-only across the boundary.
• `WorldLogger` exposes only `debug`, `info`, `warn`, and `error`, each accepting a human-readable string and optional read-only JSON object context. Transport, sink, rotation, child-loggers, and host logging configuration remain private.
• `WorldSceneRoot` is the Three.js `Group` type consumed through a type-only import. The host creates a fresh root per activation and retains ownership of host-scene attachment/removal.
• Canonical pure data shared by engine and public-host code lives under `src/shared/contracts/`; `src/shared/` is not itself world-public.
• The initial named TypeScript export set of `src/world-sdk/index.ts` is exactly: `JsonPrimitive`, `JsonValue`, `JsonObject`, `Millimeters`, `Seconds`, `MonotonicMs`, `Vec3Mm`, `Vec3MmPerSec`, `TrackingStatus`, `TrackingHealth`, `ViewerState`, `ViewportState`, `WorldSceneRoot`, `WorldAssetService`, `WorldLogger`, `WorldHostInfo`, `WorldContext`, `WorldFrame`, and `VirtualWorld`. The entrypoint must use explicit named exports; external package naming and the production ESM module protocol remain deferred.
