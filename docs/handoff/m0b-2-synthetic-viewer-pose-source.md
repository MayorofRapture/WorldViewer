...
- Known limitations / unsupported behavior: This source returns the latest scripted sample at or before a monotonic timestamp and returns null before start, after stop, and after script completion. It remains projection-free; `src/world-host/development/syntheticProjectionRuntime.ts` separately consumes it for the bounded M0B synthetic path.
