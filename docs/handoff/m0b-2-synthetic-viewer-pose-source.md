...
- Known limitations / unsupported behavior: This source returns the latest scripted sample at or before a monotonic timestamp and returns null before start, after stop, and after script completion. When sampling crosses the final scripted timestamp, the final sample is delivered once before later timestamps return null; restart resets completion state. It remains projection-free; `src/world-host/development/syntheticProjectionRuntime.ts` separately consumes it for the bounded M0B synthetic path.
